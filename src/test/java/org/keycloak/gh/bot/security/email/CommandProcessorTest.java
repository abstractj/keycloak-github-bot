...
public class CommandProcessorTest {
    ...
    public void testNewSecAlertUpdatesTitleAndSendsEmail() throws IOException {
        ...
        when(mailSender.sendNewEmail(eq(secAlertEmail), eq(targetGroup), anyString(), anyString()))
        ...
        // Fix: Expect "CVE-TBD" prefix in the subject as the processor enforces it
        verify(mailSender).sendNewEmail(
                eq(secAlertEmail),
                eq(targetGroup),
                eq("CVE-TBD Potential RCE"),
    ...
    public void testIgnoreProcessedComments() throws IOException {
        ...
        // In this test helper, we need to find the comment we just added.
        // Note: The helper creates new comments every call, but here we just need one issue with one comment.
        // The issue mock is fresh.
        ...
        // Run 2 (Should skip due to cache)
        commandProcessor.processCommands();
        verify(mailSender, times(1)).sendNewEmail(anyString(), anyString(), anyString(), anyString());
    }
    ...
    private GHIssue mockIssueWithComments(String... bodies) throws IOException {
        ...
        for (String body : bodies) {
            ...
            PagedIterator<GHReaction> reactionsIter = mock(PagedIterator.class);
            when(reactionsIter.hasNext()).thenReturn(false);
            when(reactions.iterator()).thenReturn(reactionsIter);
        ...
        // FIX: Return a NEW PagedIterator for each iterator() call.
        // This prevents the iterator from being exhausted by scanIssue() before findThreadIdInComments() can use it.
        when(iterable.iterator()).thenAnswer(invocation -> {
            PagedIterator<GHIssueComment> iterator = mock(PagedIterator.class);
            Iterator<GHIssueComment> realIterator = comments.iterator();

            when(iterator.hasNext()).thenAnswer(i -> realIterator.hasNext());
            when(iterator.next()).thenAnswer(i -> realIterator.next());
            return iterator;
        });
